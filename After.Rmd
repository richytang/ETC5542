---
title: "Back transformation after forecasting"
output:
...
```{r}
mean_predict %>%
  mutate(I = exp(I)) -> mean_predict
```

```
find_ci <- function(data, ci = 0.95){
  low_bound <- (1-ci)/2
  upper_bound <- low_bound + ci
  
  loop <- dim(data)[1]
  high_low <- matrix(NA, ncol = 2, nrow =  loop)
  
  for (i in 1:loop) {
    high_low[i,1] <- qpois(low_bound, lambda = data$I[i])
    high_low[i,2] <- qpois(upper_bound, lambda = data$I[i])
  }
  
  data["low"] = high_low[,1]
  data["high"] = high_low[,2]
...
```{r}
find_ci(data = mean_predict, ci = 0.99) -> mean_predict
```
...
full_data %>%
  ...
  geom_line(aes(x = Date, y = I,color = "Actual"),color = "grey") +
  geom_line(aes(x = Date, y = I,color = "mean", group = Week), data = mean_predict) +
  geom_point(aes(x = Date, y = I,color = "mean"),shape = 2,
             data = mean_predict, size = 0.5) +
  geom_ribbon(aes(Date, ymin = low, ymax = high, group = Week),data = mean_predict) +
  theme_bw() +
  ylab("Number of New Case") +
  ggtitle("The Mean of Forecasting Value and Confidence interval")
```

I use seasonal adjust into my model, and do back transformation. Then, I construct CI by poisson distribution. The CI is also very narrow
...
window1_predict %>%
  ...
  mutate(resid = exp(I) - fc_actual$I) %>%
